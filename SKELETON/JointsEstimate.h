...
int inline bound(int& val,int min, int max)
{
	if(val > max )
		return max;
	if(val < min)
...
void JointEstimate::analysisJ( const float *img, bool if_front, int2& Px_Torso, int2& Px_Head, int2& Px_LeftArm, int2& Px_RightArm, int2& Px_LeftLeg, int2& Px_RightLeg)
{
	...
	Y_head_up = bound(Y_head_up, 0, h - 1);
	...
	Y_head_down = bound(Y_head_down, 0, h - 1);
	...
	int hip_width;
	int2 JointC = make_int2 ( glconfig->width / 2, h / 2);
	if(record[Y_hip].size() > 0)
	int2 JointC = make_int2(abs(record[Y_hip][1]) / 2 + abs(record[Y_hip][2] / 2 ), max(Y_hip + 25, h));
